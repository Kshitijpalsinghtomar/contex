...
const PRICE_PRO = 3.50;    // Approx $3.50 / 1M input (Pro tier)
...
async function runBenchmark() {
    ...
    if (model) {
        try {
            ...
            console.error('Failed to count tokens via API:', e.message);
    ...
    const latJson = JSON.stringify(latencySubset);
...
function printReport(datasetName: string, count: number, resPretty: any, resMin: any, resTens: any) {
    ...
    console.log(`${pad('Metric', 20)} ${padL('JSON Pretty', 15)} ${padL('JSON Min', 15)} ${padL('TENS', 15)} ${padL('Improvement', 15)}`);
    console.log(`${pad('', 20)} ${padL('', 15).replace(/ /g, '-')} ${padL('', 15).replace(/ /g, '-')} ${padL('', 15).replace(/ /g, '-')} ${padL('', 15).replace(/ /g, '-')}`);

    const pctTokens = resMin.tokens > 0 ? ((1 - resTens.tokens / resMin.tokens) * 100).toFixed(1) + '%' : 'N/A';
    console.log(`${pad('Tokens', 20)} ${padL(resPretty.tokens.toString(), 15)} ${padL(resMin.tokens.toString(), 15)} ${padL(resTens.tokens.toString(), 15)} ${padL(pctTokens, 15)}`);

    if (resMin.latency > 0) {
        const timeSpeedup = (resMin.latency / resTens.latency).toFixed(1) + 'x';
        console.log(`${pad('Latency (ms)', 20)} ${padL(resPretty.latency.toFixed(0), 15)} ${padL(resMin.latency.toFixed(0), 15)} ${padL(resTens.latency.toFixed(0), 15)} ${padL(timeSpeedup, 15)}`);
    } else {
        console.log(`${pad('Latency (ms)', 20)} ${padL('N/A', 15)} ${padL('N/A', 15)} ${padL('N/A', 15)} ${padL('N/A', 15)}`);
    }
    ...
    console.log(`${pad('Gemini 1.5 Pro', 20)} ${padL('$' + costProPretty.toFixed(0), 15)} ${padL('$' + costProMin.toFixed(0), 15)} ${padL('$' + costProTens.toFixed(0), 15)} ${padL('SAVE $' + savePro.toFixed(0), 15)}`);
    ...
    console.log(`${pad('Gemini 1.5 Flash', 20)} ${padL('$' + costFlashPretty.toFixed(0), 15)} ${padL('$' + costFlashMin.toFixed(0), 15)} ${padL('$' + costFlashTens.toFixed(0), 15)} ${padL('SAVE $' + saveFlash.toFixed(0), 15)}`);
