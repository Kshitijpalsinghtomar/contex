...
const PRICE_MINI = 0.150; // $0.15 / 1M input tokens
const PRICE_STANDARD = 2.500; // $2.50 / 1M input tokens (GPT-4o)
...
async function runBenchmark() {
    ...
    console.log('Note: Using first 10 records for latency testing to avoid Rate Limits, while projecting costs for full dataset.');
...
function printReport(datasetName: string, count: number, resPretty: any, resMin: any, resTens: any) {
    ...
    console.log(`${pad('GPT-4o Cost', 20)} ${padL('$' + costStdPretty.toFixed(0), 15)} ${padL('$' + costStdMin.toFixed(0), 15)} ${padL('$' + costStdTens.toFixed(0), 15)} ${padL('SAVE $' + saveStd.toFixed(0), 15)}`);
    ...
    console.log(`${pad('GPT-4o-Mini Cost', 20)} ${padL('$' + costMiniPretty.toFixed(0), 15)} ${padL('$' + costMiniMin.toFixed(0), 15)} ${padL('$' + costMiniTens.toFixed(0), 15)} ${padL('SAVE $' + saveMini.toFixed(0), 15)}`);
